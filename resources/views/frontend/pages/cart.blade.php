...
<script>
    function inc(sel, id) {
        let inp = $(sel).parent().find('.inp');
        let val = inp.val(),
            nxt = parseFloat(val) + 1;
            max = inp.attr('max');
        if (nxt <= max) {
            inp.val(nxt);
            update_cart(sel, id, nxt);
        }
    }
    function dec(sel, id) {
        let inp = $(sel).parent().find('.inp'),
    ...
    function update_cart(sel, id, qty) {
...
<style>
/* <div class="ct-toast ct-cursor-pointer ct-toast-success" role="status" tabindex="0" style="border-left: 3px solid rgb(110, 192, 95); opacity: 0; margin-top: -15px;">
    ...
    <div class="ct-text-group">
        <div class="ct-text">This is a success message.</div>
    </div>
</div> */
#ct-container {
	position: fixed;
